...
def count_csv_rows_dask(file_path, header=True):
    ...
    return (
        row_count if header else row_count
    )  # Subtract 1 for the header row (if present)
...
def get_random_sample(file_path, fraction=0.01, output_path=None, seed=42):
    ddf = dd.read_csv(file_path, dtype="object")

    sampled_df = ddf.sample(frac=fraction, random_state=seed)

    return (
        sampled_df.compute()
        if output_path == None
        else sampled_df.to_csv(output_path, single_file=True, index=False)
    )
